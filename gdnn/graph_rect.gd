...
func plot_boolean_sub(pos:Vector2):
	var b = -1
	if ope == OP_AND: b = 1.0 if pos.x > 0 && pos.y > 0.0 else -1.0		# AND
	elif ope == OP_OR: b = 1.0 if pos.x > 0 || pos.y > 0.0 else -1.0		# OR
	elif ope == OP_NAND: b = -1.0 if pos.x > 0 && pos.y > 0.0 else 1.0		# NAND
	elif ope == OP_GT: b = 1.0 if pos.x > pos.y else -1.0					# x1 > x2
	elif ope == OP_XOR: b = 1.0 if pos.x != pos.y else -1.0					# XOR
